from etl.Utils import general
from etl.inserter import insert_data
from etl.db import connection
from etl.db import Schema
import yfinance as yf
import etl.risk_metric as rm
import os
from etl.fetcher import ingest_fundamental_data, ingest_tech_data
import pandas as pd
import time
...
def risk_metric(cursor, conn):
    ...
    risk_free = risk_free.loc['2022-04-16':]['^IRX']['Close']
    annalized_risk_free = risk_free.mean()/100 
    market_data = pd.read_sql_query(query, conn)
    staged_data = rm.risk_metric('2022-04-16', '2025-12-20', market_data, annalized_risk_free)
    print(staged_data) 
